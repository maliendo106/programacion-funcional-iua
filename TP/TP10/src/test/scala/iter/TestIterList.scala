...
class TestList extends AnyFunSuite with Checkers:
  ...
  test("(x:::xs).tail tiene que ser xs") {
    check((x: Int, xs: List[Int]) => (x ::: xs).tail == xs)
  }
  ...
  test(
    ...
    check { (args: SList[Int]) =>
      (!args.isEmpty) ==> {
        ...
        val was = List(args*).reduceRight(math.min)
        (expected == was) :| s"expected: $expected\n\t  was: $was\n    on input: $args"
      }
    }
  }
  ...
  test("List(args*).find(_ == p) debe ser equivalente a args.find(_ == p)") {
    check{(args: SList[Byte], x: Byte) =>
      val expected = args.find(_ == x)
      val was = List(args*).find(_ == x)
      (expected == was) :| s"expected: $expected\n\t  was: $was\n    on input: find $x in $args"
    }
  }
  ...
  test("List().find(_ == p) debe ser None") {
    check{(x: Int) => 
      val expected = None
      val was = List.empty[Int].find(_ == x)
      (expected == was) :| s"expected: $expected\n\t  was: $was\n    on input: find $x in List.empty"
    }
  }
  ...
  test(
    ...
    check{(args: SList[Char], s: String, sep: String, e: String) =>
      ...
      (expected == was) :| s"expected: '$expected'\n\t  was: '$was'\n    on input: $args.mkString($s, $sep, $e)"
    }
  }
end TestList
